...
def is_mark(text: str, conf: float) -> bool:
    ...
    if len(text) == 1 and 0.2 < conf < 0.4:
        return True
...
def parse_answer_for_question(
    ...
    for text_item in page_texts:
        ...
        if text in ['C', 'c', 'Đ', 'đ', 'D', 'd', 'K', 'k']:
            ...
            if question_y - 10 <= label_y <= question_y_max + 30:
    ...
    for label in label_texts:
        ...
        checkbox_ranges.append({
            'min_x': checkbox_left - 12,  # Giảm margin xuống 12px để giảm overlap hơn nữa
            'max_x': checkbox_right + 12,
            'answer': answer,
            'label_x': label_x,
            'label_y': label_y
        })
    ...
    for text_item in page_texts:
        ...
        is_near = question_y - 15 <= y <= question_y_max + 10
    ...
    for cb_range in checkbox_ranges:
        ...
        for text_item in page_texts:
            ...
            if (cb_range['min_x'] <= x <= cb_range['max_x'] and
                question_y - 15 <= y <= question_y_max + 10 and
