...
def test_model(image_path: str, model_path: str = "checkbox_model/best.pt"):
    ...
    # Resize to training size (800) if needed
    # YOLO will auto-resize, but let's be explicit
    print(f"📏 Model training size: 800")
    print()
    
    # Run inference with lower confidence threshold
    print("🔍 Running inference...")
    print("   (Trying multiple confidence thresholds...)")
    
    # Try different confidence thresholds (start from very very low)
    conf_thresholds = [0.01, 0.02, 0.03, 0.05, 0.1, 0.15, 0.2, 0.25]
    best_results = None
    best_conf = None
    
    for conf_thresh in conf_thresholds:
        results = model.predict(img, conf=conf_thresh, verbose=False, imgsz=800)
        result = results[0]
        boxes = result.boxes
        ...
        if len(boxes) > 0:
            ...
            print(f"   ⚠️  No detections at conf={conf_thresh}")
    ...
    print()
    print(f"✅ Using detections at conf={best_conf}")
