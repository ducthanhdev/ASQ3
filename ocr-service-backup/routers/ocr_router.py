...
async def parse_answers(request: ParseRequest) -> ParseResponse:
    try:
        ...
        # Try YOLO parser first, fallback to OCR parser if YOLO fails or returns too few answers
        answers = {}
        try:
            file_data = None
            ...
            yolo_answers = parse_answers_with_yolo_paddleocr(
                request.pages, 
            ...
            # Check if YOLO parser found enough answers
            # Count non-None and non-empty values
            non_empty_yolo = len([v for v in yolo_answers.values() if v is not None and v != ""])
            expected_count = len(request.question_ids)
            
            logger.info(f"YOLO parser returned {len(yolo_answers)} answers ({non_empty_yolo} non-empty) out of {expected_count} questions")
            
            if non_empty_yolo >= expected_count * 0.5:  # At least 50% answers
                answers = yolo_answers
                logger.info("Used YOLO parser, found %d answers (%d non-empty)", len(answers), non_empty_yolo)
            else:
                logger.warning(f"YOLO parser found only {non_empty_yolo}/{expected_count} answers, falling back to OCR parser")
                ocr_answers = parse_answers_from_pages(request.pages, request.question_ids)
                logger.info(f"OCR parser returned {len(ocr_answers)} answers")
                logger.debug(f"OCR answers sample: {dict(list(ocr_answers.items())[:5])}")
                # Merge: prefer YOLO answers, fill gaps with OCR
                answers = ocr_answers.copy()
                for qid, val in yolo_answers.items():
                    if val is not None and val != "":  # Only use non-None and non-empty YOLO answers
                        answers[qid] = val
                non_empty_merged = len([v for v in answers.values() if v is not None and v != ""])
                logger.info(f"Merged YOLO and OCR parsers: {len(answers)} total answers ({non_empty_merged} non-empty)")
